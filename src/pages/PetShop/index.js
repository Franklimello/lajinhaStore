...
const CACHE_KEY = "products_petshop";
const CACHE_TTL = 5 * 60 * 1000; // 5 minutos
const PRODUCTS_PER_PAGE = 20; // Limitar a 20 produtos por vez

const PetShop = memo(function PetShop({ searchTerm = "", isPreview = false }) {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    try {
      ...
      if (!isLoadMore) {
        ...
        if (cached) {
          ...
          if (Date.now() - timestamp < CACHE_TTL) {
            console.log(`✅ Cache hit: Pet Shop (${products.length} produtos)`);
            ...
            const catOptions = ["Pet shop", "Pet Shop", "pet shop", "petshop"];
            const bgQuery = query(
      ...
      const catOptions = ["Pet shop"]; // ✅ CORRIGIDO: nome EXATO do Firebase
      let q = query(
        collection(db, "produtos"),
        where("categoria", "in", catOptions),
        orderBy("titulo"),
      ...
      if (isLoadMore && lastDoc) {
        q = query(
          collection(db, "produtos"),
          where("categoria", "in", ["Pet shop"]), // ✅ CORRIGIDO
          orderBy("titulo"),
          startAfter(lastDoc),
          limit(PRODUCTS_PER_PAGE)
        );
      ...
      console.log(`✅ Carregados ${newProducts.length} produtos de Pet Shop`);
      ...
      <section className="min-h-screen bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-20">
            ...
            <p className="mt-4 text-gray-600 text-lg">Carregando produtos para pets...</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!isPreview && (
          <div className="w-full flex justify-center py-8 mb-8">
            <div className="bg-gradient-to-br from-white to-amber-50/30 rounded-2xl shadow-xl border-2 border-amber-100 p-6 w-full max-w-3xl">
              <div className="flex flex-col items-center justify-center gap-3">
                <div className="w-full relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar produtos pet shop... (ex: ração, brinquedo, casinha)"
                    ...
                    aria-label="Pesquisar produtos pet shop"
                    autoComplete="off"
                    spellCheck="false"
                    disabled={searching}
                  />
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg mb-6">
            <FaPaw className="text-amber-500 text-xl animate-pulse" />
            <span className="text-sm font-medium text-gray-600 uppercase tracking-wider">
              Tudo Para Seu Pet
            </span>
          </div>

          <h1 className="text-5xl md:text-6xl font-black bg-gradient-to-r from-amber-600 via-yellow-600 to-orange-600 bg-clip-text text-transparent mb-4">
            Pet Shop
          </h1>

          <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Produtos com carinho para seu melhor amigo
          </p>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            ...
            <span className="text-gray-600 ml-2">Produtos de qualidade para pets</span>
          </div>
          ...
          {!isPreview && localSearchTerm && localSearchTerm.length >= 3 && !searching && (
            <p className="mt-4 text-sm text-gray-600">
              {resultsCount > 0 ? (
                <span>Resultados para "<strong className="text-amber-600">{localSearchTerm}</strong>": {resultsCount} produto{resultsCount !== 1 ? 's' : ''}</span>
              ) : (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-amber-600 hover:to-orange-600 transition-all duration-300 hover:scale-105 shadow-lg"
                >
