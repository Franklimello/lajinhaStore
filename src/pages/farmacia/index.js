...
import { FaHeart, FaShoppingCart, FaStar, FaEye, FaPrescriptionBottleAlt, FaSearch } from "react-icons/fa";
import { ShopContext } from "../../context/ShopContext";
import { CartContext } from "../../context/CartContext";
import CartAddAnimation from "../../components/CartAddAnimation";

// ✅ CONSTANTES para otimização
const CACHE_KEY = "products_farmacia";
const CACHE_TTL = 5 * 60 * 1000; // 5 minutos
const PRODUCTS_PER_PAGE = 20; // Limitar a 20 produtos por vez

const Farmacia = memo(function Farmacia({ searchTerm = "", isPreview = false }) {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    try {
      ...
      const catOptions = ["Farmácia", "farmacia", "Farmacia", "farmácia"];
      let q = query(
        collection(db, "produtos"),
        where("categoria", "in", catOptions),
      ...
      console.log(`✅ Carregados ${newProducts.length} produtos de Farmácia`);
            ...
            <div className="bg-gradient-to-br from-white to-green-50/30 rounded-2xl shadow-xl border-2 border-green-100 p-6 w-full max-w-3xl">
              <div className="flex flex-col items-center justify-center gap-3">
                <div className="w-full relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar produtos de farmácia... (ex: remédio, pomada, vitamina)"
                    ...
                    aria-label="Pesquisar produtos de farmácia"
                    autoComplete="off"
                    spellCheck="false"
                    disabled={searching}
                  />
