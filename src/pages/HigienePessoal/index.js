...
import { FaHeart, FaShoppingCart, FaStar, FaEye, FaSpa, FaSearch } from "react-icons/fa";
...
const HigienePessoal = memo(function HigienePessoal({ searchTerm = "", isPreview = false }) {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    try {
      ...
      if (!isLoadMore) {
        ...
        if (cached) {
          ...
          if (Date.now() - timestamp < CACHE_TTL) {
            console.log(`✅ Cache hit: Higiene Pessoal (${products.length} produtos)`);
            ...
            const catOptions = ["Higiene pessoal"];
            const bgQuery = query(
      ...
      const catOptions = ["Higiene pessoal"]; // ✅ CORRIGIDO: nome EXATO do Firebase
      let q = query(
        collection(db, "produtos"),
        where("categoria", "in", catOptions),
      ...
      if (isLoadMore && lastDoc) {
        q = query(
          collection(db, "produtos"),
          where("categoria", "in", ["Higiene pessoal"]), // ✅ CORRIGIDO
          orderBy("titulo"),
      ...
      console.log(`✅ Carregados ${newProducts.length} produtos de Higiene Pessoal`);
      ...
      console.error("❌ Erro ao buscar produtos de higiene pessoal:", error);
      setAllProducts([]);
      setHasMore(false);
    } finally {
      setLoading(false);
  ...
  if (loading && allProducts.length === 0 && !localSearchTerm) {
    return (
      <section className="min-h-screen bg-gradient-to-br from-violet-50 via-purple-50 to-fuchsia-50 p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!isPreview && (
          <div className="w-full flex justify-center py-8 mb-8">
            <div className="bg-gradient-to-br from-white to-violet-50/30 rounded-2xl shadow-xl border-2 border-violet-100 p-6 w-full max-w-3xl">
                ...
                <div className="w-full relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
                    {searching ? (
                      ...
                      <FaSearch className="text-violet-500 text-xl" />
                    )}
                  </div>
                  <input
                    type="text"
                    placeholder="Buscar produtos de higiene... (ex: shampoo, sabonete, desodorante)"
                    ...
                    aria-label="Pesquisar produtos de higiene"
                    autoComplete="off"
                    spellCheck="false"
                    disabled={searching}
                  />
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg mb-6">
            <FaSpa className="text-violet-500 text-xl animate-pulse" />
            <span className="text-sm font-medium text-gray-600 uppercase tracking-wider">
              Cuidados Pessoais
            </span>
          </div>

          <h1 className="text-5xl md:text-6xl font-black bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 bg-clip-text text-transparent mb-4">
            Higiene Pessoal
          </h1>

          <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Produtos para sua higiene e cuidados pessoais diários
          </p>
          ...
          {!isPreview && localSearchTerm && localSearchTerm.length >= 3 && !searching && (
            <p className="mt-4 text-sm text-gray-600">
              {resultsCount > 0 ? (
                <span>Resultados para "<strong className="text-violet-600">{localSearchTerm}</strong>": {resultsCount} produto{resultsCount !== 1 ? 's' : ''}</span>
              ) : (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-violet-500 to-purple-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-violet-600 hover:to-purple-600 transition-all duration-300 hover:scale-105 shadow-lg"
                >
                ...
                {group.map(product => (
                  <SwiperSlide key={product.id}>
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden bg-gradient-to-br from-violet-100 to-purple-100 aspect-square">
                        <img
                      ...
                      <div className="p-6">
                        ...
                        <h3 className="text-sm font-medium text-gray-800 mb-2 line-clamp-2 group-hover:text-violet-600 transition-colors duration-200 uppercase">
                          ...
                          className={`w-full py-3 rounded-xl font-semibold text-white transition-all duration-300 transform relative overflow-hidden ${
                            product.esgotado
                              ...
                              : hoveredProduct === product.id
                                ? 'bg-gradient-to-r from-violet-500 to-purple-500 shadow-lg scale-105'
        ...
        {isPreview && allProducts.length > 10 && (
          <div className="text-center mt-8">
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                window.location.href = '/higiene-pessoal';
              }}
              className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-500 to-violet-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-purple-600 hover:to-violet-600 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl"
            >
        ...
        {!isPreview && !hasMore && allProducts.length > 0 && (
          <div className="text-center mt-8 py-6 bg-white/50 backdrop-blur-sm rounded-2xl shadow-md">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Total: <span className="font-bold text-purple-600">{allProducts.length}</span> produtos
            </p>
          </div>
        )}
