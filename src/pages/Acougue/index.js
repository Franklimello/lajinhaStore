...
import { FaHeart, FaShoppingCart, FaStar, FaEye, FaDrumstickBite, FaSearch } from "react-icons/fa";
import { ShopContext } from "../../context/ShopContext";
import { CartContext } from "../../context/CartContext";
import MeatCutModal from "../../components/MeatCutModal";
import CartAddAnimation from "../../components/CartAddAnimation";

// ✅ CONSTANTES para otimização
const CACHE_KEY = "products_acougue";
const CACHE_TTL = 5 * 60 * 1000; // 5 minutos
const PRODUCTS_PER_PAGE = 20; // Limitar a 20 produtos por vez

const Acougue = memo(function Acougue({ searchTerm = "", isPreview = false }) {
  ...
  // Estados para o modal de corte de carne
  const [isMeatCutModalOpen, setIsMeatCutModalOpen] = useState(false);
  const [selectedProduct, setSelectedProduct] = useState(null);

  // Função para abrir o modal de corte
  const handleOpenMeatCutModal = (product) => {
    setSelectedProduct(product);
    setIsMeatCutModalOpen(true);
  };

  // Função para adicionar ao carrinho com o tipo de corte
  const handleAddToCartWithCut = (cutType) => {
    if (selectedProduct) {
      // Adiciona o produto ao carrinho com informação extra do corte
      const productWithCut = {
        ...selectedProduct,
        meatCut: cutType, // Informação do corte
        observacao: `Corte: ${cutType}` // Também como observação para fácil visualização
      };
      addToCart(productWithCut);
      // Incrementa contador e mostra animação a partir da segunda vez
      const currentCount = productClickCounts[selectedProduct.id] || 0;
      const newCount = currentCount + 1;
      setProductClickCounts(prev => ({ ...prev, [selectedProduct.id]: newCount }));
      if (newCount >= 2) {
        setAnimatingProducts(prev => ({ ...prev, [selectedProduct.id]: newCount }));
        setTimeout(() => {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    try {
      ...
      if (!isLoadMore) {
        ...
        if (cached) {
          ...
          if (Date.now() - timestamp < CACHE_TTL) {
            console.log(`✅ Cache hit: Açougue (${products.length} produtos)`);
            ...
            const catOptions = ["Açougue", "acougue"];
            const bgQuery = query(
      ...
      console.log(`✅ Carregados ${newProducts.length} produtos de Açougue`);
  ...
  if (loading && allProducts.length === 0 && !localSearchTerm) {
    return (
      <section className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-amber-50 p-4 md:p-8">
  ...
  return (
    <section className="min-h-screen mt-10 bg-gradient-to-br from-red-50 via-orange-50 to-amber-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        
        {/* SearchBar MELHORADA - apenas quando não for preview */}
        {!isPreview && (
          <div className="w-full flex justify-center py-8 mb-8">
            <div className="bg-gradient-to-br from-white to-red-50/30 rounded-2xl shadow-xl border-2 border-red-100 p-6 w-full max-w-3xl">
              <div className="flex flex-col items-center justify-center gap-3">
                <div className="w-full relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar carnes... (ex: alcatra, picanha, frango)"
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg mb-6">
            <FaDrumstickBite className="text-red-500 text-xl animate-pulse" />
            <span className="text-sm font-medium text-gray-600 uppercase tracking-wider">
              Carnes Premium
            </span>
          </div>

          <h1 className="text-5xl md:text-6xl font-black bg-gradient-to-r from-red-600 via-orange-600 to-amber-600 bg-clip-text text-transparent mb-4">
            Açougue
          </h1>

          <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Carnes frescas e de qualidade selecionadas com cuidado
          </p>
          ...
          {!isPreview && localSearchTerm && localSearchTerm.length >= 3 && !searching && (
            <p className="mt-4 text-sm text-gray-600">
              {resultsCount > 0 ? (
                <span>Resultados para "<strong className="text-red-600">{localSearchTerm}</strong>": {resultsCount} produto{resultsCount !== 1 ? 's' : ''}</span>
              ) : (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-red-500 to-orange-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-red-600 hover:to-orange-600 transition-all duration-300 hover:scale-105 shadow-lg"
                >
                      ...
                      <div className="p-6">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            e.stopPropagation();
                            if (!product.esgotado) {
                              handleOpenMeatCutModal(product);
                            }
                          }}
        ...
        {isPreview && allProducts.length > 10 && (
          <div className="text-center mt-8">
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                window.location.href = '/acougue';
              }}
              className="inline-flex items-center gap-2 bg-gradient-to-r from-red-500 to-orange-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-red-600 hover:to-orange-600 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl"
            >
          ...
          <div className="flex justify-center mt-8">
            ...
            >
              {loadingMore ? (
                ...
                <>
                  <FaDrumstickBite className="text-xl pointer-events-none" />
        ...
        {!isPreview && !hasMore && allProducts.length > 0 && (
          <div className="text-center mt-8 py-6 bg-white/50 backdrop-blur-sm rounded-2xl shadow-md">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Total: <span className="font-bold text-red-600">{allProducts.length}</span> produtos
            </p>
          </div>
        )}
      ...
      {/* Modal de Corte de Carne */}
      <MeatCutModal
        isOpen={isMeatCutModalOpen}
        onClose={() => setIsMeatCutModalOpen(false)}
        onConfirm={handleAddToCartWithCut}
        productName={selectedProduct?.titulo || ""}
      />
    </section>
  );
});
