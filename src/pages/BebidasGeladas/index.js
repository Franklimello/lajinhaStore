...
import { FaHeart, FaShoppingCart, FaStar, FaEye, FaSnowflake, FaSearch } from "react-icons/fa";
...
const BebidasGeladas = memo(function BebidasGeladas({ searchTerm = "", isPreview = false }) {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    try {
      ...
      const catOptions = ["Bebidas Geladas", "bebidas geladas", "Bebida Gelada", "bebida gelada", "Geladas", "geladas"];
      let q = query(
        collection(db, "produtos"),
        where("categoria", "in", catOptions),
      ...
      console.log(`✅ Carregados ${newProducts.length} produtos de Bebidas Geladas`);
      ...
      console.error("❌ Erro ao buscar produtos de bebidas geladas:", error);
      setAllProducts([]);
      setHasMore(false);
    } finally {
      setLoading(false);
  ...
  if (loading && allProducts.length === 0 && !localSearchTerm) {
    return (
      <section className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-indigo-50 p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-20">
            <div className="inline-block animate-spin rounded-full h-16 w-16 border-b-2 border-blue-500"></div>
            <p className="mt-4 text-gray-600 text-lg">Carregando bebidas geladas...</p>
          </div>
        </div>
      </section>
    );
  ...
  const structuredData = {
    ...
    "@type": "CollectionPage",
    "name": "Bebidas Geladas - Supermercado Online Lajinha",
    "description": "Bebidas geladas e refrescantes para os dias mais quentes em Lajinha-MG. Refrigerantes, sucos gelados, energéticos e muito mais.",
    "url": "https://compreaqui.com.br/bebidas-geladas",
    "mainEntity": {
      "@type": "ItemList",
      "name": "Bebidas Geladas Lajinha",
      "description": "Produtos de bebidas geladas e refrescantes em Lajinha-MG",
  ...
  return (
    <>
      <SEO
        title="Bebidas Geladas Lajinha - Supermercado Online"
        description="Bebidas geladas e refrescantes para os dias mais quentes em Lajinha-MG. Refrigerantes, sucos gelados, energéticos, água gelada e muito mais. Entrega rápida no Supermercado Online Lajinha!"
        keywords="bebidas geladas Lajinha, refrigerantes gelados Lajinha, sucos gelados Lajinha, energéticos Lajinha, água gelada Lajinha, bebidas refrescantes Lajinha, supermercado online Lajinha, entrega rápida Lajinha MG"
        url="/bebidas-geladas"
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {!isPreview && (
            <div className="w-full flex justify-center py-8 mb-8">
              <div className="bg-gradient-to-br from-white to-blue-50/30 rounded-2xl shadow-xl border-2 border-blue-100 p-6 w-full max-w-3xl">
                <div className="flex flex-col items-center justify-center gap-3">
                  <div className="w-full relative">
                    ...
                    <input
                      type="text"
                      placeholder="Buscar bebidas geladas... (ex: coca, suco, água gelada)"
                      ...
                      aria-label="Pesquisar bebidas geladas"
                      autoComplete="off"
                      spellCheck="false"
                      disabled={searching}
                    />
          ...
          <div className="inline-flex items-center gap-3 bg-white/80 backdrop-blur-sm px-6 py-3 rounded-full shadow-lg mb-6">
            <FaSnowflake className="text-blue-500 text-xl animate-pulse" />
            <span className="text-sm font-medium text-gray-600 uppercase tracking-wider">
              Bebidas Geladas
            </span>
          </div>

          <h1 className="text-5xl md:text-6xl font-black bg-gradient-to-r from-blue-600 via-cyan-600 to-indigo-600 bg-clip-text text-transparent mb-4">
            Bebidas Geladas
          </h1>

          <p className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Bebidas geladas e refrescantes para os dias mais quentes
          </p>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            ...
            <span className="text-gray-600 ml-2">Bebidas geladas de qualidade</span>
          </div>
          ...
          {!isPreview && localSearchTerm && localSearchTerm.length >= 3 && !searching && (
            <p className="mt-4 text-sm text-gray-600">
              {resultsCount > 0 ? (
                <span>Resultados para "<strong className="text-blue-600">{localSearchTerm}</strong>": {resultsCount} produto{resultsCount !== 1 ? 's' : ''}</span>
              ) : (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-cyan-600 transition-all duration-300 hover:scale-105 shadow-lg"
                >
                ...
                {group.map(product => (
                  <SwiperSlide key={product.id}>
                    ...
                    >
                      ...
                      <div className="relative overflow-hidden bg-gradient-to-br from-blue-100 to-cyan-100 aspect-square">
                        {(() => {
                      ...
                      <div className="p-6">
                        ...
                        <h3 className="text-sm font-medium text-gray-800 mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors duration-200 uppercase">
        ...
        {isPreview && allProducts.length > 10 && (
          <div className="text-center mt-8">
            <button
              ...
              onClick={(e) => {
                e.preventDefault();
                window.location.href = '/bebidas-geladas';
              }}
              className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-6 py-3 rounded-xl font-semibold hover:from-blue-600 hover:to-cyan-600 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl"
            >
        ...
        {!isPreview && !hasMore && allProducts.length > 0 && (
          <div className="text-center mt-8 py-6 bg-white/50 backdrop-blur-sm rounded-2xl shadow-md">
            ...
            <p className="text-sm text-gray-600 mt-2">
              Total: <span className="font-bold text-blue-600">{allProducts.length}</span> produtos
            </p>
          </div>
        )}
      </div>
