...
const Hortifruti = memo(function Hortifruti({ searchTerm = "", isPreview = false }) {
  ...
  const fetchProducts = async (isLoadMore = false) => {
    try {
      ...
      if (!isLoadMore) {
        ...
        if (cached) {
          ...
          if (Date.now() - timestamp < CACHE_TTL) {
            console.log(`✅ Cache hit: Hortifruti (${products.length} produtos)`);
      ...
      console.log(`✅ Carregados ${newProducts.length} produtos de Hortifruti`);
            ...
            <div className="bg-gradient-to-br from-white to-green-50/30 rounded-2xl shadow-xl border-2 border-green-100 p-6 w-full max-w-3xl">
              <div className="flex flex-col items-center justify-center gap-3">
                <div className="w-full relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar hortifruti... (ex: banana, tomate, alface)"
                    ...
                    aria-label="Pesquisar hortifruti"
                    autoComplete="off"
                    spellCheck="false"
                    disabled={searching}
                  />
